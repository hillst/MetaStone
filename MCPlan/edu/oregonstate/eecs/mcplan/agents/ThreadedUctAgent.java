 ...
 */
public final class ThreadedUctAgent extends Agent {
...
/*    *//** Method for quickly simulating entire game. *//*
    ...
    private abstract class Node {
         ...
         */
        public synchronized void update(int[] rewards) {
            if (rewards_ == null)
         ...
         *             if visits_ < 1.
         */
        public synchronized int getReward(int agentId) {
    ...
    private class StateNode<S extends State, A> extends Node {
         ...
         */
        public synchronized ActionNode<S, A> uctSelect() {
     ...
     */
    public ThreadedUctAgent(int nSimulations, double uctConstant, ExecutorService executor) {
        if (nSimulations < 1)
    ...
    public ThreadedUctAgent(int nSimulations, double uctConstant, ExecutorService executor, Agent basePi) {
        this(nSimulations, uctConstant, executor);
        basePolicy_ = basePi;
        name_ = "UCT of " + basePolicy_.getName();
    }
     ...
     *            if equal to -1.
     */
    public ThreadedUctAgent(int nSimulations, double uctConstant, ExecutorService executor, int sparseSampleSize) {
        this(nSimulations, uctConstant, executor);
    ...
    public ThreadedUctAgent(int nSimulations, double uctConstant, ExecutorService executor, int sparseSampleSize, Agent basePi) {
        this(nSimulations, uctConstant, executor, sparseSampleSize);
        basePolicy_ = basePi;
        name_ = "UCT of " + basePolicy_.getName();
    }
    ...
    public ThreadedUctAgent(int nSimulations, double uctConstant, ExecutorService executor, int sparseSampleSize,
        ...
        this(nSimulations, uctConstant, executor,  sparseSampleSize, ensembleTrials, ensembleMethod);
        basePolicy_ = basePi;
        name_ = "UCT of " + basePolicy_.getName();
    }
    ...
    public ThreadedUctAgent(int nSimulations, double uctConstant, ExecutorService executor, int sparseSampleSize,
        ...
        this(nSimulations, uctConstant, executor, sparseSampleSize, ensembleTrials, ensembleMethod, basePi);
        this.filter_ = filter;
    }
    ...
    public <S extends State, A> A selectAction(S state,
        ...
        for (int i = 0; i < nEnsembles_; i++) {
            ...
            List<Callable<Object>> todo = new ArrayList<Callable<Object>>(nSimulations_);

            for (int j = 0; j < nSimulations_; j++)
                todo.add(Executors.callable(new PlaySimulationWorker(root, iSimulator.copy())));
    ...
    private class PlaySimulationWorker implements Runnable{
        private StateNode root;
        private Simulator iSimulator;
        public PlaySimulationWorker(StateNode root, Simulator iSimulator){
            this.root = root;
        ...
        @Override
        public void run(){
            playSimulation(this.root, this.iSimulator);
        }
    }
    //dicks
