 ...
 **/
public class PolicyRollout extends Agent {
	/* The base policy. */
	...
	public PolicyRollout(Agent basePolicy, int width, int height){
	...
	public PolicyRollout(Agent basePolicy, int width, int height, ActionFilter filter, Heuristic heuristic) {
		this(basePolicy, width, height);
		this.filter_ = filter;
	...
	public <S extends State, A> A selectAction(S state,
		...
		System.out.println("policy chose: " + actions.get(best));
	...
	public <S extends State, A> double[] qEstimates(S state, Simulator<S, A> iSimulator) {
		...
		for(int i = 0; i < actions.size(); i++){
			...
			for(int j = 0; j < width_; j++){
				...
				simulator.takeAction(actions.get(i));
				int reward = simulator.getReward(policyID);
				System.out.println("policy rollout on action: " + actions.get(i));
